...
4. macOS 관리자 창이 나타나면 승인합니다.
5. 작업이 끝나면 토글을 끕니다.

토글은 Codex나 Claude의 실행 여부와 무관합니다. 두 프로그램이 모두 꺼져
있어도 토글이 켜져 있으면 Mac의 시스템과 네트워크는 계속 작동합니다.
디스플레이는 별도로 꺼질 수 있습니다.

관리자 승인을 취소하거나 덮개 닫힘 모드를 시작하지 못하더라도, 화면이 열린
상태의 기본 잠자기 방지는 계속 활성화됩니다. 메뉴에는 현재 적용된 모드가
`기본 잠자기 방지` 또는 `덮개 닫힘 허용`으로 표시됩니다.

## 안전장치

덮개가 열려 있을 때의 유휴 잠자기는 macOS IOKit power assertion으로
막습니다. 덮개를 닫은 상태에서는 관리자 권한으로 `SleepDisabled` 설정을
임시 적용하는 가디언이 함께 실행됩니다.
