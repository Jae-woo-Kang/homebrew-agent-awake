...
프로세스를 실행하지 않으므로 즉시 전환됩니다.

`Mac 잠자기 방지`는 사용자가 아무 작업을 하지 않아 발생하는 자동 시스템
잠자기를 막습니다. 덮개를 닫거나 Apple 메뉴에서 직접 잠자기를 선택하는
동작은 막지 않습니다.

## 잠자기 방지 방식

macOS의 사용자 수준 IOKit power assertion을 사용합니다. 시스템 전원 설정을
바꾸거나 관리자 권한을 요청하지 않습니다. 토글을 끄거나 AgentAwake를 종료하면
assertion이 즉시 해제되어 macOS의 기본 잠자기 동작으로 돌아갑니다.
